...
from src.models import Question, GPTResponse, ComparisonResult, QuestionResult, ExamResult
...
class Processor:
    ...
    def process_exam(self, exam_name: str = None, start_num: int = None, end_num: int = None) -> ExamResult:
        logger.info(f"Starting processing of exam '{exam_name}'")
        exam_start_time = datetime.now()

        questions = self.data_loader.load_questions(exam_name, start_num, end_num)
        if not questions:
            logger.warning(f"No questions found for exam '{exam_name}'")
            return None
        ...
        for idx, question in enumerate(tqdm(questions, desc=f"Processing {exam_name}", unit="question"), start=1):
            logger.info(f"Processing question {idx}/{len(questions)} (ID: {question.id})")
            try:
                # Record start time
                ...
                is_correct = gpt_response.selected_answer.upper() == question.correct_answer.upper()
                ...
                logger.info(f"Question '{question.id}' processed: Correct={is_correct}, Time={question_duration:.2f}s")
        ...
        logger.info(f"Completed processing of exam '{exam_name}' with {len(questions_results)} results")
        logger.info(f"Exam duration: {exam_duration:.2f}s, Accuracy: {exam_result.accuracy:.2%}")
    ...
    def process_all_exams(self) -> List[ExamResult]:
        ...
        logger.info(f"Completed processing all exams. Processed {len(results)} exams successfully")
        return results
